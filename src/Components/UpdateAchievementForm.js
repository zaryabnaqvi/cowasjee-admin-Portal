...
const UpdateAchievementForm = ({id}) => {
...
year:'',
  ...
  const getFormData = async () => {
    try {
      const response = await fetch(`https://nedmob1.neduet.edu.pk:8080/achievement/${id}`, {});
  
      if (response.ok) {
        // Show toast achievement on success
        toast.success('achievement retrieved successfully!');
        ...
        const data = await response.json();
        console.log(data);
        throw new Error('Failed to retrieve achievement');
      }
    } catch (error) {
      console.error('Error:', error);
      toast.error('Failed to retrieve achievement');
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    formDataToSend.append("achievement_id",id)
    ...
    try {
      const response = await fetch('https://nedmob1.neduet.edu.pk:8080/update-achievement', {
        method: 'PATCH',
        headers: {
      ...
      if (response.ok) {
        ...
        const data = await response.json()
        console.log(data)
        throw new Error('Failed to post achievement');
      }
    } catch (error) {
      console.error('Error:', error);
...
let flag=0
  ...
  return (
    <div className="">
        ...
        <div className="flex items-center justify-center my-3">
          <div style={{boxShadow: "rgba(50, 50, 93, 0.25) 0px 50px 100px -20px, rgba(0, 0, 0, 0.3) 0px 30px 60px -30px, rgba(10, 37, 64, 0.35) 0px -2px 6px 0px inset",borderRadius:"10px"}} className="xl:mx-auto shadow-lg p-5 xl:w-full xl:max-w-lg 2xl:max-w-md">
            ...
            <h2 className="text-2xl font-bold leading-tight">
              Update Posted achievement
            </h2>
            <p className="mt-2 text-base text-gray-600">
              Note: achievements you update will be displayed on the main Cowasjee website.
            </p>
      ...
      <ToastContainer /> {/* Toast container for displaying achievements */}
    </div>
  );
};
